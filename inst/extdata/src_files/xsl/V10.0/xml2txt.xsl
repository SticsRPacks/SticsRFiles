...
<!-- Introducting new level template for structuring varietal parameters -->
<xsl:template match="formalismev">
...
<!-- Introducting new template call for varietal parameters -->
<xsl:template match="variete">
<xsl:text>codevar
</xsl:text><xsl:value-of select="@nom" /><xsl:text>
</xsl:text>
<xsl:apply-templates select="formalismev | param | optionv"/>
</xsl:template>
...
<xsl:apply-templates select="colonne/@nom"/>
<xsl:text>
</xsl:text>
...
<xsl:template match="colonne/@nom">
<xsl:value-of select="."/><xsl:text> </xsl:text>
</xsl:template>
