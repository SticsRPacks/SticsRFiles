...
tmp1 <- unlist(get_param_txt(workspace = path, param = "stlevamf",
                             stics_version = stics_version))
codevar <- unlist(get_param_txt(workspace = path, param = "codevar",
                                stics_version = stics_version))
tmp2 <- unlist(get_param_txt(workspace = path, param = "stlevamf",
                             variety = codevar, stics_version = stics_version))
variete <- unlist(get_param_txt(workspace = path, param = "variete",
                                stics_version = stics_version))
tmp3 <- unlist(get_param_txt(workspace = path, param = "stlevamf",
...
varieties <- get_param_txt(workspace = path,
                        stics_version = stics_version)$plant$plant1$codevar[1:3]
#tmp1 <- as.numeric(get_param_txt(workspace = path, param = "stlevamf",
# variety = varieties))
tmp1 <- get_param_txt(workspace = path, param = "stlevamf",
                      variety = varieties,
                      stics_version = stics_version)$plant$plant1$stlevamf
tmp2 <- get_param_txt(workspace = path,
                      stics_version = stics_version)$plant$plant1$stlevamf[1:3]
...
tmp <- unlist(get_param_txt(workspace = path, param = "forme", exact = TRUE,
                            stics_version = stics_version))
set_param_txt(
...
tmp2 <- unlist(get_param_txt(workspace = path, param = "forme", exact = TRUE,
                             stics_version = stics_version))
...
tmp <- unlist(get_param_txt(workspace = path, param = "stlevamf",
                            stics_version = stics_version))
set_param_txt(
...
tmp <- unlist(get_param_txt(workspace = path, param = "stlevamf", variety = 4,
                            stics_version = stics_version))
set_param_txt(
  workspace = path,
  param = "stlevamf",
...
tmp2 <- unlist(get_param_txt(workspace = path, param = "stlevamf", variety = 4,
                             stics_version = stics_version))
test_that("Set and get of a varietal parameter for a unique plant
          for a given variety", {
  expect_equal(tmp + 1, tmp2)
...
path <- file.path(get_examples_path("txt", stics_version = stics_version),
