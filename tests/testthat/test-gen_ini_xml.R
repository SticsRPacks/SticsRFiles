...
if (get_version_num(stics_version = stics_version) >= 10) stics_version <- "V10.0"
parnames <- list()
...
gen_ini_xml(param_df = ini_param[1, ], out_dir = out_dir, stics_version = stics_version)
...
xl_params <- gsub(pattern = "(.*)_(.*)",x = names(xl_plt1_values), replacement = "\\1")
xl_params <- gsub(pattern = "(.*)_(.*)",x = xl_params, replacement = "\\1")
xl_plt1_values <- select(xl_plt1_values, starts_with(xl_params))
xml_plt1_values <- get_param_xml(file = ini_xml, select = "plante", select_value = 1)[[1]]
...
xml_plt2_values <- get_param_xml(file = ini_xml, select = "plante", select_value = 2)[[1]]
...
xml_hinit_values <- unlist(get_param_xml(file = ini_xml, param = hinit_name)[[1]])

xl_hinit_values <- select(ini_param[1, ], starts_with(hinit_name))

# NO3init
no3init_name <- parnames$no3init[[stics_version]]
xml_NO3init_values <- unlist(get_param_xml(file = ini_xml, param = no3init_name)[[1]])
xl_NO3init_values <- select(ini_param[1, ], starts_with(no3init_name))

# NH4init
nh4init_name <- parnames$nh4init[[stics_version]]
xml_NH4init_values <- unlist(get_param_xml(file = ini_xml, param = nh4init_name)[[1]])
...
test_that("Testing values for plant parameters", {
  ...
  expect_equal(all(xml_plt2_values %in% c("","-999")), ncol(xl_plt2_values) == 0)
})
