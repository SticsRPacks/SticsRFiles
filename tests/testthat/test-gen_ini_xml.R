...
xl_path <- file.path(get_examples_path("xl", stics_version = stics_version),
...
xml_plt2_values <- get_param_xml(file = ini_xml, select = "plante",
                                 select_value = 2)[[1]]
...
xml_hinit_values <- unlist(get_param_xml(file = ini_xml,
                                         param = hinit_name)[[1]])
...
xml_NO3init_values <- unlist(get_param_xml(file = ini_xml,
                                           param = no3init_name)[[1]])
xl_NO3init_values <- select(ini_param[1, ], starts_with(no3init_name))

# NH4init
nh4init_name <- parnames$nh4init[[stics_version]]
xml_NH4init_values <- unlist(get_param_xml(file = ini_xml,
                                           param = nh4init_name)[[1]])
...
test_that("Testing values for plant parameters", {
  ...
  expect_equal(all(xml_plt2_values %in% c("", "-999")),
