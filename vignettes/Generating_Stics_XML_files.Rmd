...
* In this part information are focusing on functionnalities related to Stics XML files generation capabilities based on parameterization for lists of simulations (USMs) stored in tabulated data files; as Excel or CSV files.
...
* The generation of wheather files files is not included yet in the package, but will be included in a future version. One can use the tool provided by JavaStics GUI to perform these files generation.
...
* gen_tec_xml: for generating crop management files (`*_ini.xml`)
...
usm_xl_file <- download_usm_xl(dest_dir = xl_dir)
...
usm_xl_file <-  download_usm_xl(xl_name = "inputs_stics_example.xlsx", dest_dir = workspace_path)
```
...
usm_csv_file <- download_usm_csv(csv_name = "inputs_stics_example_USMs.csv", dest_dir = csv_dir )
...
```{r, eval=TRUE, echo = FALSE, results='hide'}
usm_csv_file <- download_usm_csv(csv_name = "inputs_stics_example_USMs.csv", dest_dir = workspace_path)
```
...
Sheets content and CSV files content is the same, only their use is different. But, CSV files are easily traced under verion control systems and they also can be generated programmatically with the help of functions dedicated for example to generating crop management parameters like irrigation, fertilization (interventions and doses calculations).
...
* For generating new files, containing parameters with differents values for each USM, tables **must contain values for all the files parameters**.
...
Excel sheet names are completely free, but must be meaningfull regarding the kind of parameters set they contain.

In the example, usefull sheets for generating XML parameters files are named: **USMs, Ini, Soils, Tec, Station**
...
Other columns of tables are named with **parameters names** according to the fololowing rules:
...
Here is the actual **correspendance table** of `keywords` to be used in parameters tables in replacement of real names:
...
For the moment, *no consistency checks* (between Excel sheets or CSV equivalents files) functionnalities have been included in the package in order to prevent missing generated files or misspelled file names or identifiers.
...
# xl_param <- read_excel(usm_xl_file,sheet = "USMs",col_types = c(rep("guess",13),rep("text", 4) ), na="NA")

xl_param <- read_params_table(usm_xl_file,sheet_name = "USMs")

#xl_param[is.na(xl_param)] <- ""
...
gen_usms_xml(usms_out_file = out_file, usms_param = usms_param)
```
...
# Output file path
out_file <- "/path/to/file/sols.xml" # or something like C:/path/to/file/usms.xml" for Windows

# Generating a new sols.xml file, for all xl_param lines
gen_sols_xml(sols_out_file = out_file, sols_param = soils_param)
```
...
gen_tec_xml(param_table = tec_param, out_path = workspace_path )
```
...
l <- readLines(con = file.path(workspace_path,"USM_2017_T1_CI_tec.xml"))
...
gen_ini_xml(param_table = ini_param, out_path = workspace_path )
```
...
gen_sta_xml(param_table = sta_param, out_path = workspace_path )
```
