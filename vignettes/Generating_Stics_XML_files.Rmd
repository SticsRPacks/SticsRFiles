...
# Getting example files and scripts

## Getting inputs `Excel` example file: 
...
```{r}
# a specific
copy_mailing_script_example("generate_usms_file.R")
copy_mailing_script_example("generate_usms_file.R", dest_dir = "/path/to/dest/dir")
...
```
# Example for generating the usms XML file

## `generate_usms_file.R`
...
## Adaptation of the script content
   ...
   * Only adapt the **paths / directories** and **files names**. In these scripts, the Excel file is given just as an example. It will be replaced by the user ones.
...
* All the parameters are not mandatory in the Excel sheet, only those that are to be modified
