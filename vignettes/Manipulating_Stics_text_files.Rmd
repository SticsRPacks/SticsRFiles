...
```{r}
example_txt_dir <- get_examples_path( file_type = "txt", stics_version = "V9.0")
```


## Getting parameter value

`get_param_txt()` helps to get the value of a given parameter in a USM. It has two arguments, the directory of the USM (`workspace`, where the text files lives), and the parameter name (`param`).
...
```{r}
get_param_txt(workspace = example_txt_dir, param='a')
```
...
For example to set the value of the atmospheric pressure read above to 1000:
