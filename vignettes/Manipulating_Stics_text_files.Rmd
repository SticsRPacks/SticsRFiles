...
`get_param_txt()` helps to get the value of a given parameter in a USM. It has two arguments, the directory of the USM (`workspace`, where the text files lives), and the parameter name (`param`). An additional optional argument, is to be used if one want to get parameters for another Stics version than the latest one, as for getting the example dirtectory: `stics_version` 
