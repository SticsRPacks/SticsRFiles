...
knitr::kable(dt, caption = "Correspondence between files names and parameters groups")
...
The `get_param_info` only take the names or names substring of a parameter or a parameters list and by default searches in an XML files list according to a model version (9.1 as the default one).
...
# Searching formalisms from files

<!-- mentionning that as default behaviour that get_param_info returns also formalisms taht param
is attached to, see if usefulle to keep this section ????? --> 

The `get_param_info` as `get_param_info`, only take the names or names substring of a parameter or a parameters list and by default searches in an XML files list according to a model version (9.1 as the default one).
...
## Finding formalisms using partial formalism names (description !)
...
param_formalisms <- get_param_info(parameter = c("yield","leaves"))

get_param_info(param_formalisms)

```

## Finding formalisms using partial formalism and parameters names (description !)
...
# 
# tec_param_values <- get_param_xml(tec, param_name = tec_param_names)[[1]]
# 
...
This is also already handled for getting parameters values from files of the same kind.
