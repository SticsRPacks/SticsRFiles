...
get_param_info <- function(param = NULL,
  ...
  if (lifecycle::is_present(version)) {
    lifecycle::deprecate_warn("1.0.0",
                              "get_param_info(version)",
                              "get_param_info(stics_version)")
  } else {
  ...
  if (all(c(any(parform_use), keyword_use))) {
    stop("Incompatible search using both parameter and/or ",
         "formalism words and keywords !")
  }
  ...
  if (keyword_use) {
    ...
    lines_filter <-
      param_data_df$formalism %in% form_names |
      param_data_df$file %in% file_names |
