...
#' @export
#'
get_obs= function(workspace=getwd(), usms_list=NULL, usms_filename="usms.xml",javastics_path = NULL,verbose=TRUE){

  # For a list of folders recurive call
  if(length(workspace) > 1){
    obs_list <- lapply(workspace, function(x) get_obs(x,usms_list, usms_filename))
  ...
  # Getting plant names:
  plant_names= get_plant_name(workspace, usms_list, usms_filename, javastics_path, verbose)
